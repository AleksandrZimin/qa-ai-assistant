...
description: Общий справочник по инженерии ПО (архитектура, frontend, backend, тестирование/автоматизация, DevOps/CI-CD, security, debugging, TDD, код-ревью). Используй ТОЛЬКО когда тестировщик явно просит помощь по коду/автоматизации/архитектуре/CI-CD/безопасности — не для обычных QA-задач (тестирование требований, тест-кейсы, выгрузка в Test IT), для них есть отдельные скиллы testing-requirements/test-case-design/testit-sync. Пригодится в будущем при развитии проекта в сторону автоматизации.
license: MIT
metadata:
  author: get-zeked
  version: '1.0'
...
> **Как читать этот файл.** Это адаптированный справочник, изначально написанный для среды Perplexity Computer, объединённый с набором инженерных скиллов Claude Code. Три момента, важных для этого проекта:
> 1. Все команды вида `python scripts/*.py` ниже — иллюстративные паттерны из первоисточника. В этом репозитории таких скриптов нет, не пытайся их запускать — используй как справочный пример подхода, а не как реальный доступный инструмент.
> 2. Раздел 1 ("Gap Analysis") сравнивает возможности Perplexity Computer и Claude Code — это контекст происхождения документа, а не описание текущего проекта.
> 3. Сейчас этот проект сфокусирован на ручном тестировании (см. CLAUDE.md). Используй этот скилл только если тестировщик явно спрашивает про код, архитектуру, автоматизацию, CI/CD или безопасность — не применяй проактивно к обычным QA-запросам.
