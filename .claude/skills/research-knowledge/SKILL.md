...
description: Справочник по методологии исследований, работе с источниками, статистике, визуализации данных и проверке качества данных/выводов (включая чек-лист Pre-Delivery QA для аналитики). Используй для задач, выходящих за рамки стандартного ручного тестирования — глубокое исследование темы, анализ данных, оценка достоверности источников. Для типовых QA-задач (требования, тест-кейсы, Test IT) есть отдельные скиллы testing-requirements/test-case-design/testit-sync.
license: MIT
metadata:
  author: get-zeked
  version: '1.0'
...
> **Как читать этот файл.** Адаптированный справочник, изначально написанный для среды Perplexity Computer. Упоминания Perplexity Computer/Claude Code ниже (особенно в разделе 1 "Gap Analysis") — сравнение возможностей источника, а не описание текущего проекта. Команды вида `python scripts/*.py`, `yt-dlp`, внешние интеграции и т.п. — иллюстративные паттерны из первоисточника, в этом репозитории их нет и агент не должен пытаться их вызывать.
